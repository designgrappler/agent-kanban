...
| S13-T2 | Empty state + Add-default-agent flow — zero-state CTAs, "Add backlog items" auto-creates a default board then routes to its backlog, "Recruit an agent" → Default agents picker, custom team-member form (avatar local-file upload + Agent-OS template fields) | Code close-gate  | Bridge issued — depends on T1 ✅ (unblocked) |
