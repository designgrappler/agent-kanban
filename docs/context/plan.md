...
## Sprint 9 — Backlog Tab (OPENED 2026-05-21)
...
| S9-T3 | Frontend Backlog page | Skylar  | VERIFIED — awaiting commit (E2E waived) | `/boards/:id/backlog` route, components, hook, Playwright spec        |
...
- [ ] Migration `apps/web/migrations/0026_backlog_items.sql` applied locally and committed
- [ ] `backlogRepo.ts` thin repo layer (no raw SQL in routes)
- [ ] Hono routes: `POST /api/boards/:id/backlog-items`, `GET /api/boards/:id/backlog-items`, `PATCH /api/backlog-items/:id`, `DELETE /api/backlog-items/:id`
- [ ] `ROUTE_RULES` updated: write routes → `["user", "agent:leader"]`, GET inherits board-read rule
- [ ] Shared types added to `packages/shared/src/` and consumed by web + CLI
- [ ] CLI subcommand `ak backlog` implementing `add | list | update | delete`
- [ ] Backlog page route `/boards/:id/backlog` registered in the web router
- [ ] Components shipped: `BacklogPage`, `BacklogItemCard`, `BacklogItemForm`
- [ ] Data hook `useBacklogItems` (load, mutate, optimistic update)
- [ ] Priority-grouped rendering (P0/P1/P2/P3 sections, manual reorder within section)
- [ ] Backlog entry point in board header — adjacent to board switcher
- [ ] Playwright spec covers create / edit / delete flows on the Backlog page
- [ ] Vitest unit tests for `backlogRepo` and the four route handlers (auth, validation, happy path, error envelope)
- [ ] Full regression green: `pnpm build && pnpm tsc --noEmit && npx vitest run`
- [ ] All three tracks merged to `main` via PR

_Sprint 9 block last updated: 2026-05-21 (S9-T1/T2 DONE Bandit PASS; S9-T3 VERIFIED gates green, E2E waived per Tim, awaiting merge)_
...
*Last updated: 2026-05-21 (Sprint 9 OPENED — Backlog tab plan finalized; six "Decisions Needed" approved by Tim 2026-05-21; three Handoff Bridges issued for S9-T1/T2/T3. Sprint 8 CLOSED — see archive below.)*
